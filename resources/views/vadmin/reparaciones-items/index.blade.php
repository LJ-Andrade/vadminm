...
{{-- PAGE TITLE--}}
@section('title', 'Vadmin | Reparacionesitems')

{{-- HEAD--}}
@section('header')
	@section('header_title', 'Listado de Reparacionesitems') 
	@section('options')
		<div class="actions">
            <a href="{{ url('vadmin/reparacionesitems/create') }}" class="btn btnSm buttonOther">Nueva</a>
...
@section('content')
    <div class="container">
		<div class="row">		
			@include('vadmin.reparacionesitems.searcher')
            <div class="col-md-12 animated fadeIn main-list">
                @foreach($reparacionesitems as $item)
                <div id="Id{{ $item->id }}" class="Item-Row Select-Row-Trigger row item-row simple-list">
                    ...
                    <div class="List-Actions lists-actions Hidden">
						<a href="{{ url('/vadmin/reparacionesitems/' . $item->id . '/edit') }}" class="btnSmall buttonOk" data-id="{{ $item->id }}">
							<i class="ion-ios-compose-outline"></i>
						</a>
						<a href="{{ url('vadmin/reparacionesitems/'. $item->id) }}" class="btnSmall buttonOther">
